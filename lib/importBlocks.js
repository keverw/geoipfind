					...
					readCSV(
						...
						function line(handler, lineNum, data)
						{
							if (!hadErr)
							{
								...
								{
									var address = (version == 4) ? new Address4(data[0]) : new Address6(data[0]);
								...
								else
								{
									hadErr = true;
									handler.close();
									return rollbackOnErr(new Error('Incorrect IP version'));
								}
						...
						function end()
						{
							if (!hadErr)
							{
								var timer = setInterval(function()
								{
									...
									{
										clearInterval(timer);
										logFN('Committing changes to geo_blocks table');
