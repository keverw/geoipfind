var readCSV = require('./readCSV.js'),
	...
	module.exports = function(db, logFN, verbose, path, lang, o_done_cb)
	{
		var statements = { //statements to create
			...
			geo_namesInsert: 'INSERT OR IGNORE INTO geo_names (geoname_id, type, continent_code, country_iso_code, subdivision_1_iso_code, subdivision_2_iso_code, metro_code, time_zone) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
			langCheck_select: 'SELECT field, code FROM lang WHERE field = ? AND code = ?',
			lang_insert: 'INSERT OR IGNORE INTO lang (`field`, `code`, `' + lang + '`) VALUES (?, ?, ?)',
											...
											if (row) //already inserted
											{
												processLang(langData, function(err)
												{
													...
													else
													{
														...
														var precent = parseInt(((dbInsertsFinished / totalLines) * 100));
